...
import entity.AnalysisSludge;
import entity.Process;

import com.bstek.bdf2.core.orm.hibernate.HibernateDao;

@Component
public class AnalysisSludgeDao extends HibernateDao {
	@DataProvider
	public Collection<AnalysisSludge> loadAnalysisIn(Integer id) throws Exception {
		if(id != null){
			String hql="from " + AnalysisSludge.class.getName();
...
@DataProvider
public void queryPage(Page<AnalysisSludge> page,Map<String, Object> param) throws Exception{
		...
		String sql="from "+AnalysisSludge.class.getName()+ whereCase + " order by date desc , processId asc" ;
	    this.pagingQuery(page, sql,"select count(*) " + sql);
	}
	
	
	@DataResolver
	public void saveAnalysisSludges(Collection<AnalysisSludge> analysisSludges) throws Exception {
		Session session = this.getSessionFactory().openSession();
		try{
			for(AnalysisSludge analysisSludge : analysisSludges){
				EntityState state = EntityUtils.getState(analysisSludge);
				if(state.equals(EntityState.NEW)){
					session.save(analysisSludge);
				}
				if(state.equals(EntityState.MODIFIED)){
					//Date now = new Date();
					//analysisIn.setTimestamp(now);
					session.update(analysisSludge);
				}
				if(state.equals(EntityState.DELETED)){
					session.delete(analysisSludge);
				}
			}
		} finally{
			session.flush();
